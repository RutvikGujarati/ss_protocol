    ...
    ReentrancyGuard // IERC20 initialization
{
    ...
    function _distributeCycleAllocations(
        uint256 stateLPShare,
    ...
    function getMintTimestamps(
        ...
        external
        view
        returns (uint256[] memory mintTimes, uint256[] memory expireTimes)
    {
        MintBatch[] storage batches = mintBatches[user];
   ...
   caching or snapshots to maintain clear, straightforward logic, accepting the gas cost as a trade-off for simplicity and transparency. */
    ...
    function getPendingTokenNames(
        ...
        require(
            userTokenCount[user] <= MAX_TOKEN_PER_USER,
        ...
        for (uint256 i = 0; i < totalTokens; i++) {
            ...
            if (
                userTokenEntries[user][allTokens[i]].status ==
                TokenStatus.Pending
            ) {
    ...
    function burnState(uint256 amount) external whenNotPaused {
        ...
        require(
            ...
            "Burning not allowed yet. Timer hasn't started."
        );
        require(
    ...
    function getClaimablePLS(address user) public view returns (uint256) {
        ...
        for (uint256 i = 0; i < userUnclaimedCycles[user].length; i++) {
            ...
            assert(
                userCycleBurned[user][cycle] > 0 || hasClaimedCycle[user][cycle]
            ); // either burned or already claimed
    ...
    function getUserSharePercentage(
        ...
        return
            (userCycleBurned[user][previousCycle] * BASIS_POINTS) /
            cycleTotalBurned[previousCycle];
    }

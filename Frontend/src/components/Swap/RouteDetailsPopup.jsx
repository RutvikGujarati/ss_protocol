...
const RouteDetailsPopup = ({ routeDetails, showRoutePopup, setShowRoutePopup, getTokenLogo, TOKENS, state }) => {
    return (
        <div
                    ...
                    <div className="d-flex flex-column gap-3">
                        {routeDetails?.paths?.length > 0 && routeDetails?.swaps?.length > 0 ? (
                            routeDetails.paths.map((path, i) => (
                                <div key={i} className="d-flex align-items-center gap-3 flex-wrap">
                                    ...
                                    <div className="d-flex flex-row gap-3 flex-wrap align-items-center" style={{ flex: 1, minWidth: 0 }}>
                                        <div className="d-flex flex-row gap-3 flex-wrap align-items-center justify-content-center" style={{ flex: 1, minWidth: 0 }}>
                                            {path.map((token, idx) => {
                                                ...
                                                return (
                                                    <React.Fragment key={idx}>
                                                        ...
                                                        >
                                                            <div className="d-flex align-items-center mb-1" style={{ gap: 2, fontSize: "0.78rem" }}>
                                                                {TOKENS[token.symbol]?.symbol === "STATE" ? (
                                                                    <img src={state} alt="STATE" width="12" height="12" style={{ borderRadius: "50%" }} />
                                                                ) : TOKENS[token.symbol]?.image && TOKENS[token.symbol]?.image.startsWith('http') ? (
                                                                    <img src={TOKENS[token.symbol].image} alt={token.symbol} width="12" height="12" style={{ borderRadius: "50%" }} />
                                                                ) : TOKENS[token.symbol]?.emoji ? (
                                                                    <span style={{ fontSize: "0.78em" }}>{TOKENS[token.symbol].emoji}</span>
                                                                ) : (
                                                                    <img src="/default.png" alt={token.symbol} width="12" height="12" style={{ borderRadius: "50%" }} />
                                                                )}
                                                                ...
                                                                {TOKENS[nextToken.symbol]?.symbol === "STATE" ? (
                                                                    ...
                                                                    <img src={TOKENS[nextToken.symbol].image} alt={nextToken.symbol} width="12" height="12" style={{ borderRadius: "50%" }} />
                                                                ) : TOKENS[nextToken.symbol]?.emoji ? (
                                                                    <span style={{ fontSize: "0.78em" }}>{TOKENS[nextToken.symbol].emoji}</span>
                                                                ) : (
                                                                    <img src="/default.png" alt={nextToken.symbol} width="12" height="12" style={{ borderRadius: "50%" }} />
                                                                )}
                                                        ...
                                                        {idx < path.length - 2 && (
                                                            <div style={{ width: 10, height: 2, borderBottom: "2px dotted #555", margin: "0 2px" }}></div>
                                                        )}
                                                    </React.Fragment>
                                                );
                                            })}

...
import { useContext } from "react";
import {
	$1, $10, Currus, CurrusRatioAddress, DAV_TOKEN_ADDRESS, Domus,
	DomusRatioAddress, Fluxin, OneDollarRatioAddress, Ratio_TOKEN_ADDRESS,
	Rieva, RievaRatioAddress, STATE_TOKEN_ADDRESS, TenDollarRatioAddress,
...
export const TokensDetails = () => {
	...
	return tokenConfigs.map(([name, displayName, key, addr, supply, price, extra]) => {
		...
		return {
			tokenName: name, key, name: displayName,
			supply: supply || simpleSupplies[`${contract}Supply`],
			Supply: isDAV ? Supply : simpleSupplies[`${contract}Supply`],
			Price: price,
			address: addr,
			SwapContract: swapContracts[contract],
			SwapShortContract: shortAddrs[swapKey],
			...
			percentage: swap.decayPercentages[contract],
			stateBalance: swap.balances[`State${contract}`],
			target: swap.RatioTargetsofTokens[contract],
			isReversing: swap.isReversed[contract]?.toString(),
			WillStart: swap.ReverseForCycle[contract],
			WillStartForNext: swap.ReverseForNextCycle[contract],
			Balance: swap.balances[`${contract}Balance`],
			TotalTokensBurn: TotalTokensBurned[contract],
			RatioBalance: swap.balances?.[`ratio${contract}Balance`],
			Duration: auctionDetails[contract],
			interval: auctionDetails[contract],
			AuctionRunning: AuctionRunningLocalString?.[contract]?.toString(),
			pair: isState ? undefined : `${name}/pSTATE`,
			Ratio: CurrentRatioPrice[contract],
			AuctionTimeRunning: auctionTimeLeft[contract],
			AuctionNextTime: auctionDetails[contract],
			mintAddTOkens: extra?.mint,
